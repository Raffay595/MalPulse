...
export default function GooeyNav({
  ...
  const updateEffectPosition = (element: HTMLLIElement) => {
    ...
    // Grab just the text label, skipping the icon span
    textRef.current.innerText = element.querySelector("a")?.innerText ?? element.innerText
  }
  ...
  const handleClick = (e: React.MouseEvent<HTMLLIElement>, index: number) => {
    ...
    // Navigate after a short delay so the animation is visible
    setTimeout(() => {
      router.push(items[index].href)
    }, 120)
  }
